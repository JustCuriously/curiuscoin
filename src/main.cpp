...
#if defined(NDEBUG)
# error "Curiuscoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x80998e8651dfb230cb7387d03810192566c7c84a8849b6980c20211cc9369000");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Curiuscoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Curiuscoin Signed Message:\n";
...
{
    // Curiuscoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Curiuscoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Curiuscoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Curiuscoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Curiuscoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Curiuscoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xa33bab16c2c201dd0a1e4702f08465545c87856dd5ab5a6c7ab3eb7348411a23");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04dbb30547fcf3e5b806cfa60b544fe855f8c85ef55baeddee9b886c6145f98bd958e64f3ff5568182fe0a173db264db8a4d777ad9bffccf7660712fb24645df71") << OP_CHECKSIG;
        CBlock block;
        ...
        block.nNonce   = 2087183741;

        if (fTestNet)
        {
            block.nTime    = 1519991509;
            block.nNonce   = 385526266;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xeda3202c61f3edfca210ac6cb2d0b3b23d4bb8f06b4478f9a9be581c8505dac8"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Curiuscoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CuriuscoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CuriuscoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CuriuscoinMiner(CWallet *pwallet)
{
    printf("CuriuscoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("curiuscoin-miner");
    ...
    try { loop {
        ...
        printf("Running CuriuscoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CuriuscoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CuriuscoinMiner, pwallet));
}
